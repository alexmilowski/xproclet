...
",
   element atom:id {
      concat($request-host,$app.path,"/term/",$term,"/")
   },
   "
",
   element atom:link {
   ...
   "
",
   (for $e in collection('app://collections/entry')/atom:entry[atom:category[not(@scheme)][@term=$term]]
      order by $e/atom:updated descending return $e)[1]/atom:updated,
   "
",
   let $istart := xs:integer($start), $ilimit := $istart+xs:integer($limit)
   for $e in (for $e in collection('app://collections/entry')/atom:entry[atom:category[not(@scheme)][@term=$term]] order by $e/atom:updated return $e)[position()&gt;=$istart and position()&lt;$ilimit]
